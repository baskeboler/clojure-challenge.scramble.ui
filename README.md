...
* [`src/clojure_challenge/scramble/ui/styles.cljs`](src/clojure_challenge/scramble/ui/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/clojure_challenge/scramble/ui/`](src/clojure_challenge/scramble/ui/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/clojure_challenge/scramble/ui/core.cljs): contains the SPA entry point, `init`
* [`test/clojure_challenge/scramble/ui/`](test/clojure_challenge/scramble/ui/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `clojure-challenge.scramble.ui` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/clojure_challenge/scramble/ui/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns clojure-challenge.scramble.ui.example
  (:require [clojure-challenge.scramble.ui.config :as config])
